						...
						<li class="sheet-item">
							...
							<div class="button-container">
								...
								" class="btn btn-primary" @click="addSheetToMongoDB(sheet)">
									Ajouter sur la page publique
								</button>
								<button v-else class="btn btn-danger" @click="removeSheetFromMongoDB(sheet)">
									Supprimer de la page publique
								</button>
							</div>
						</li>
					</ul>
...
const addSheetToMongoDB = async sheet => {
	try {
		const response = await fetch(
			`${import.meta.env.VITE_BACKEND_URI}/sheets`,
			{
				method: 'POST',
				headers: {
...
const removeSheetFromMongoDB = async sheet => {
	try {
		const response = await fetch(
			`${import.meta.env.VITE_BACKEND_URI}/sheets/${sheet.qInfo.qId}`,
			{
				method: 'DELETE',
				headers: {
					'Content-Type': 'application/json',
